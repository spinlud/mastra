---
title: "Channels"
description: "Connect Mastra agents to Slack, Discord, Telegram, and other messaging channels with webhooks, thread context, tool approvals, and formatted replies."
packages:
  - "@mastra/core"
---
...
We recommend configuring [storage](/docs/storage) for channels. Storage preserves channel and memory state across restarts, including thread subscriptions and tool approvals:
...
```typescript title="src/mastra/agents/your-agent.ts" oxfmt:false
channels: {
  adapters: {
    slack: createSlackAdapter(),
  },
