...
description: "Share one Mastra memory thread across multiple users by preserving speaker identity in messages and combining history with observational or working memory."
packages:
  - "@mastra/core"
  - "@mastra/libsql"
  - "@mastra/memory"
...
If you switch the Observer to a less capable model and see facts attributed to a generic `User` instead of individual participants, use [`observation.instruction`](/reference/memory/observational-memory) to explain how to interpret the `<turn>` tag.
