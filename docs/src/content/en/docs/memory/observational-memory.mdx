...
description: "Configure Mastra Observational Memory to compress long conversations into durable observations while preserving context across threads and sessions."
packages:
  - '@mastra/core'
  - '@mastra/memory'
  - '@mastra/oracledb'
...
Resource scope automatically disables async buffering.
...
Because hooks receive `threadId` and `resourceId`, you can also use them to update [working memory](/docs/memory/working-memory) via `memory.updateWorkingMemory()` during a cycle. These external updates aren't atomic with the OM text commit.
