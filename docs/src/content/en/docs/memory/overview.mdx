---
title: "Memory"
description: "Add memory to Mastra agents with message history, semantic recall, working memory, and observational memory across threads and multi-agent delegation."
packages:
  - "@mastra/core"
  - "@mastra/libsql"
  - "@mastra/memory"
...
<Steps>
  ...
  <StepItem>
    Call your agent, for example in [Studio](/docs/studio/overview). Inside Studio, start a new chat with your agent and take a look at the right sidebar. It'll now display memory details.
  </StepItem>
</Steps>
...
Long conversations can fill the context window with raw message history and reduce agent performance. [Observational Memory](../memory/observational-memory) uses background agents to compress older messages into observations. This reduces the context used by message history while retaining information for later turns.
