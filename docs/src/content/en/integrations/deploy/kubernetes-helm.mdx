...
description: "Deploy Mastra Enterprise to Kubernetes with the official Helm chart, configure secrets and storage, expose services securely, and scale workloads."
---
...
The chart makes the variables available to your application. It doesn't configure observability or send telemetry by itself. Configure `ObservabilityStorageClickhouseVNext` for the `observability` storage domain and add `MastraStorageExporter` to your Mastra application. See [ClickHouse](/integrations/databases/clickhouse#observability-with-vnext-recommended) for the complete application configuration.
...
:::warning Use a new tag for each build
