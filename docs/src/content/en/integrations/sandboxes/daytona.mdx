...
description: "Run Mastra workspace commands in Daytona cloud sandboxes with custom images, resources, volumes, snapshots, streaming output, and lifecycle controls."
packages:
  - "@mastra/daytona"
---
...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'string',
    description:
      'Default directory for command execution when no per-command `cwd` is given. A per-command `cwd` always wins. When set, the sandbox skips its automatic working-directory probe; when omitted, the probe fills the `workingDirectory` getter after start.',
    isOptional: true,
  },
  {
    name: 'labels',
