...
description: "Use AgentController Session to manage per-conversation identity, threads, mode, model, run state, grants, and display snapshots within an isolated runtime."
packages:
  - "@mastra/core"
...
When `requestContext` carries `MASTRA_MESSAGE_AUTHOR_KEY`, the stored message names that sender under `content.providerMetadata.mastra.author`. See [Reserved keys](/docs/server/request-context#reserved-keys).
...
Merge a partial update into the state. The queue applies concurrent calls in order and validates them against the schema before emitting `state_changed` with the changed keys.
...
Run an updater against the current snapshot and apply its result atomically within the write queue. Use this for read-modify-write changes that must see the latest state. The updater returns mergeable `updates` and optional `events`, plus the `result` value that `update()` resolves to.
...
Configured storage persists threads and messages along with token usage, while thread settings restore mode and model choices. Restored settings can include observational memory and subagent model selections with per-agent-type overrides. A chat channel can map back to stored threads. However, `AgentControllerChannels` keeps channel-to-session and auto-approval state in memory.
