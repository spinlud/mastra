...
description: "Use createGraphRAGTool() to build semantic relationships between document chunks and retrieve connected context for graph-based RAG queries."
packages:
  - "@mastra/core"
  - "@mastra/rag"
---
...
<PropertiesTable
  content={[
  {
    ...
    type: 'string[]',
    description:
      "Array of chunk text strings for the most relevant document chunks, in rank order, retrieved using graph-based ranking. Text is read from the `text` field of each chunk's metadata.",
  },
  {
    name: 'sources',
    type: 'QueryResult[]',
