...
description: "The createVectorQueryTool() function creates a tool for semantic search over vector stores."
packages:
  - "@mastra/core"
  - "@mastra/pg"
  - "@mastra/rag"
...
<PropertiesTable
  content={[
  {
    ...
    type: 'any[]',
    description:
      'Array of metadata objects for the most relevant chunks, in rank order (one entry per result). The chunk text is available at `relevantContext[i].text` when it was stored in metadata during ingestion.',
  },
  {
    name: 'sources',
    type: 'QueryResult[]',
